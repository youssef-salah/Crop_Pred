...
app = Flask(__name__)
quality_predict = joblib.load('Crop_Prediction1.joblib')
print(type(quality_predict))

@app.route('/Crop_pred', methods=['POST', 'GET'])
def handle_data():
   try:
...
# Create response with CORS headers
      response = make_response(jsonify({'Crop_Prediction': predicted_water_quality[0]}))
